...
export default function JokeShow({joke, setJoke, setJokeOpen, ...props}): JSX.Element {
  const isConnected = useContext(IsConnectedContext);
  const [thisJoke, setThisJoke] = useState<JokeObject>(joke);
  ...
  const renderTranslationForm = (): JSX.Element => {
    if (isConnected) {
      if (!joke) return <TranslationForm joke={thisJoke} setJoke={setThisJoke} />

      return <TranslationForm joke={joke} setJoke={setJoke} />
    }
  }
  ...
  const renderTranslations = (): JSX.Element => {
    ...
    if (!joke) {
      filteredTrans = languageFilter ?
        thisJoke.translations.filter(trans => trans.language == languageFilter.value)
        : thisJoke.translations;
    } else {
      filteredTrans = languageFilter ?
  ...
  if (!joke && !thisJoke) return <p className="mt-20">Loading</p>
        ...
        <div className="md:w-1/2 bg-red-900 flex flex-col items-center px-4 py-4 sm:px-12 md:px-4 w-full">
          ...
          <div className="whitespace-pre-wrap w-full bg-white px-4 py-3 rounded-md border border-gray-200 shadow mt-6 md:mt-16">
            <div className="flex items-center">
              ...
              <p className="ml-2 font-bold">{thisJoke.author}</p>
            </div>
          ...
          <div className="bg-white w-60 rounded-full mt-2">
            <JokeReaction joke={thisJoke} setJoke={setThisJoke} isSmall={true}/>
          </div>
